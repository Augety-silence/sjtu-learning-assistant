#!/usr/bin/env python3
"""Phase 1A: verify SJTU Mail IMAP SSL access and list recent messages.

Passwords are read from macOS Keychain when available. A password entered with
getpass is written to Keychain only after IMAP authentication succeeds.
"""
...
class MailCheckError(RuntimeError):
    """A user-facing error raised during the IMAP check."""
...
def connect_and_fetch(
    ...
    try:
        ...
        raise MailCheckError(
            f"无法连接 {IMAP_HOST}:{IMAP_PORT}：{exc}"
        ) from exc
